...
bool NDTRegistration::SetRegistrationParam(float res, float step_size, float trans_eps, int max_iter) {
    ndt_ptr_->setResolution(res);                  //   网格大小设置
    ndt_ptr_->setStepSize(step_size);         //   牛顿法优化的最大步长
    ndt_ptr_->setTransformationEpsilon(trans_eps);     //  连续变换之间允许的最大差值
    ndt_ptr_->setMaximumIterations(max_iter);     //  迭代最大次数
...
bool NDTRegistration::ScanMatch(const CloudData::CLOUD_PTR& input_source, 
    ...
    ndt_ptr_->setInputSource(input_source);     //  设置原始点云
    ndt_ptr_->align(*result_cloud_ptr, predict_pose);      //   设置目标点云
    result_pose = ndt_ptr_->getFinalTransformation();    //    获取最终的配准的转化矩阵
    return true;
}
