...
bool LIOBackEnd::InitGraphOptimizer(const YAML::Node& config_node) {
    ...
    graph_optimizer_config_.use_gnss = config_node["use_gnss"].as<bool>();                              // gnss单向边
    graph_optimizer_config_.use_loop_close = config_node["use_loop_close"].as<bool>();                  // 闭环双向边
    graph_optimizer_config_.use_imu_pre_integration = config_node["use_imu_pre_integration"].as<bool>();// imu预积分双向边
    graph_optimizer_config_.use_odo_pre_integration = config_node["use_odo_pre_integration"].as<bool>();// 编码器双向边
