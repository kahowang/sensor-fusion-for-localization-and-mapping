...
/**
 * @brief  update pre-integrator state: mean, covariance and Jacobian   更新预积分状态：名义值、方差、雅克比
 * @param  void
 * @return void
 */
void IMUPreIntegrator::UpdateState(void) {
    ...
    static Eigen::Vector3d w_mid = Eigen::Vector3d::Zero();                //      角速度_中值
    static Eigen::Vector3d a_mid = Eigen::Vector3d::Zero();                 //      加速度_中值
    ...
    const IMUData &prev_imu_data = imu_data_buff_.at(0);        //  k 时刻imu数值
    const IMUData &curr_imu_data = imu_data_buff_.at(1);        //   k+1  时刻imu数值

    // get time delta:
    T = curr_imu_data.time - prev_imu_data.time;                           //   两次预积分时间差 
