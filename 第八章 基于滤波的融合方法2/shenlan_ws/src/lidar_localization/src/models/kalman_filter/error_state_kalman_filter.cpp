...
ErrorStateKalmanFilter::ErrorStateKalmanFilter(const YAML::Node &node) {
  ...
  /*init  debug print file */
  #ifdef    DEBUG_PRINT
      char fused_path[] = "/home/kaho/shenlan_ws/src/lidar_localization/slam_data/trajectory/fused_without_cons.txt";
      ...
      CreateFile(fused_vel, fused_vel_path );
  #endif 
...
bool ErrorStateKalmanFilter::Correct(const IMUData &imu_data,                 //  修正
  ...
  if (time_delta > -0.05) {                 //  时间对齐
    ...
    /*print  vel(body)*/
      ...
      WriteText(fused_vel, vel_print_ );   // 写进文件夹
  #endif
