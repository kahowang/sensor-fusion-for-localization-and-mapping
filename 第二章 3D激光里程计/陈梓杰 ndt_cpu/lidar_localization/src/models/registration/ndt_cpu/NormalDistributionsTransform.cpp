...
NormalDistributionsTransform<PointSourceType, PointTargetType>::NormalDistributionsTransform()
{
	...
	gauss_c1 = 10.0 * (1 - outlier_ratio_); // 还不懂怎么算的
...
void NormalDistributionsTransform<PointSourceType, PointTargetType>::computeTransformation(const Eigen::Matrix<float, 4, 4> &guess)
{
	...
	Eigen::Vector3f init_translation = eig_transformation.translation(); // 初始平移估计
	Eigen::Vector3f init_rotation = eig_transformation.rotation().eulerAngles(0, 1, 2); // 初始旋转估计
 ...
 * @brief 
 * 
 * @tparam PointSourceType 
 * @tparam PointTargetType 
 * @param[in] score_gradient 
 * @param[in] hessian 
 * @param[in] trans_cloud 经过initial guess变换后的点云
 * @param[in] pose t_x t_y t_z roll pitch yaw
 * @param[in] compute_hessian bool是否计算hessian矩阵
 * @return double 返回score得分
 */
template <typename PointSourceType, typename PointTargetType>
																							...
																							Eigen::Matrix<double, 6, 1> pose, bool compute_hessian)
{
	...
	for (int idx = 0; idx < source_cloud_->points.size(); idx++) {
		...
		for (int i = 0; i < neighbor_ids.size(); i++) {
			...
			x_trans -= voxel_grid_.getCentroid(vid); // x_trans - 体素质心(也就是均值)
			c_inv = voxel_grid_.getInverseCovariance(vid); // 获取体素的协方差矩阵
