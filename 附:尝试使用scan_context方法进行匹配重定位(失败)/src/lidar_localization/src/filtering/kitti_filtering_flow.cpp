...
bool KITTIFilteringFlow::Run() {
  ...
  while (HasData()) {
    if (!HasInited()) {
      ...
      if (HasLidarData() && ValidLidarData()) {             //  有imu数据和lidar数据
        if (HasIMUData()) {
          while (HasIMUData() && ValidIMUData() &&
                 current_imu_raw_data_.time < current_cloud_data_.time) {     //  有imu数据，lidar数据，且imu数据 慢于 当前点云
            UpdateLocalization();
          }

          if (current_imu_raw_data_.time >= current_cloud_data_.time) {        //   如果当前imu数据，快于 点云 ，放到队列末尾
            imu_raw_data_buff_.push_back(current_imu_raw_data_);
          }
        }

        CorrectLocalization();        //  correct 中使用的imu_data 为10hz imu data
      }
