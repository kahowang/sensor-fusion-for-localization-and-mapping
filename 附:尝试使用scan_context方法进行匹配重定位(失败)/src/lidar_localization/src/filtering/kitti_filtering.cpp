...
bool KITTIFiltering::InitOri(Eigen::Matrix4f &init_pose,   CloudData &cloud_data ,  Eigen::Matrix3f &init_ori ){        //  use gnss and though ndt get  init_ori
      init_pose.block<3,3>(0,0)  =   Eigen::Matrix3f::Identity();
      Eigen::Matrix4f  predict_pose =  init_pose;      //   不断变换预测位姿， 每次旋转 5度
      ...
      Eigen::Matrix4f  temp_init_pose  =  Eigen::Matrix4f::Identity();     //  ndt 匹配得到的变换矩阵
       ...
       for(int i = 1; i <  iter_rate + 1;  i++){     //  分成72份
          ...
          // matching: 
            ...
            std::cout  << "i = " <<  i  <<" angle =  "<<  (angle * 57.3 ) << "  score  =  "   <<   score  <<std::endl; 
            if(score < 0.1){break;}   // 得分小于 0.1 跳出循环
       }

      predict_pose.block<3,3>(0,0)  =  temp_init_pose.block<3,3>(0,0);    
    //  // matching: 
      CloudData::CLOUD_PTR result_cloud_ptr(new CloudData::CLOUD());    
      registration_ptr_->ScanMatch(filtered_cloud_ptr, predict_pose,
                                  result_cloud_ptr, temp_init_pose);     
...
bool KITTIFiltering::InitOri(Eigen::Matrix4f &init_pose,   CloudData &cloud_data ,  Eigen::Matrix3f &init_ori ){        //  use gnss and though scancontext to  get  init_ori
      init_pose.block<3,3>(0,0)  =   Eigen::Matrix3f::Identity();
      Eigen::Matrix4f  predict_pose =  init_pose;      //   不断变换预测位姿， 每次旋转 5度
      ...
      CloudData  transformed_scan_;         //   旋转遍历的点云
      CloudData   local_map_;                        //   分割出的局部地图 
      CloudData   global_map_;                        //   分割出的全局地图 
      local_map_.cloud_ptr  =  local_map_ptr_;

      /*test  scan_context*/
      Eigen::Matrix4f  translation_test  =  Eigen::Matrix4f::Identity();     //  ndt 匹配得到的变换矩阵
      CloudData  transformed_scan_test;         //   test旋转遍历的点云
      Eigen::AngleAxisf rotation_vector_test(0,  Eigen::Vector3f(0,0,1))  ;   // 绕z轴旋转  360  180  120
      Eigen::Matrix3f    rotation_test =  rotation_vector_test.matrix();
      translation_test.block<3,3>(0,0) = rotation_test.block<3,3>(0,0);
      pcl::transformPointCloud(*filtered_cloud_ptr,   *transformed_scan_test.cloud_ptr, translation_test);               
      /*test  scan_context*/
      // std::cout  <<  "local_map_z   "   << std::endl;
      // double  z;
      //   for(int i =0;  i < local_map_.cloud_ptr->points.size()  ;  i++){
      //         z  = local_map_.cloud_ptr->points.at(i).z  ;  //+ 2.0f;
      //         std::cout <<  z  <<  "\t"  ;   
      //   }
      // std::cout << "\n"  << std::endl;

      Eigen::Matrix4f  translation  =  Eigen::Matrix4f::Identity();     //  ndt 匹配得到的变换矩阵
       int iter_rate  = 360;
       for(int i = 1; i <  iter_rate + 1;  i++){     //  分成72份
            ...
            scan_context_manager_ptr_->SaveInitOri(transformed_scan_);              //  计算遍历后的sc描述子，并存储
          if(i == 60){
      ...
      std::cerr << "Saved " << (*local_map_ptr_).points.size()  << "local_map.pcd " << std::endl;
      std::cerr << "Saved " << (*transformed_scan_.cloud_ptr).points.size()  << "cur_scan.pcd " << std::endl;
      ...
      init_ori.block<3,3>(0,0)  =  translation.block<3,3>(0,0);     //  更新init  pose的航向
}
...
bool KITTIFiltering::Correct(const IMUData &imu_data,
  ...
  step_pose = last_pose.inverse() * cloud_pose;         //  上一帧点云和当前帧点云的相对位姿
  predict_pose = cloud_pose * step_pose;                     //  下一帧点云的预测位姿
  last_pose = cloud_pose;

  // shall the local map be updated:   匹配之后判断是否需要更新局部地图
...
bool KITTIFiltering::InitScanContext(const YAML::Node &config_node) {
    // get loop closure config:
  loop_closure_method_ = config_node["loop_closure_method"].as<std::string>();
