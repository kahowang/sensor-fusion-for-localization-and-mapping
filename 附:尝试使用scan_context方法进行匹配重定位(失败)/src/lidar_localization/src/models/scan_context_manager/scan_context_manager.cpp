...
std::pair<int, float> ScanContextManager::GetScanContextMatch(
    ...
    Eigen::MatrixXf target_sector_key = GetSectorKey(target_scan_context);         // Sector-Key生成函数，提取的是列向量的均值
    Eigen::MatrixXf source_sector_key = GetSectorKey(source_scan_context);

    /*sc  debug*/
    // std::cout << "旋转"  << std::endl;
    // std::cout <<  target_sector_key  <<  std::endl;
    // std::cout << "待匹配"  << std::endl;
    // std::cout << source_sector_key <<  std::endl;
    std::cout  << "sc_matrix_cur_scan  =   "    <<std::endl;
    Eigen::MatrixXf   sc_matrix(target_scan_context.rows(), target_scan_context.cols());
    for(int did = 0; did < target_scan_context.rows();  ++ did){
        for(int sid = 0; sid < target_scan_context.cols(); ++sid){
            sc_matrix(did,sid)  =   target_scan_context(did,sid);
            std::cout  <<  target_scan_context(did,sid)  << "\t"  ;
        }std::cout  << "\n"  ; 
    }
    ...
    int optimal_shift = 0;
    float optimal_dist = std::numeric_limits<float>::max();                 //  编译器允许返回的float最大值
...
bool ScanContextManager::UpdateIndex(const int MIN_KEY_FRAME_SEQ_DISTANCE) {
    if (
        ...
        state_.index_.data_.key_frame_.clear();
        state_.index_.data_.key_frame_.insert(                    //  void insert(iterator loc, input_iterator start, input_iterator end)
...
std::pair<int, float> ScanContextManager::GetLoopClosureMatch(
    ...
    // 
    if(
        state_.ring_key_.size() <= (static_cast<size_t>(MIN_KEY_FRAME_SEQ_DISTANCE_))              //  回环检测判断条件1：当前帧与历史帧需要隔比较长时间
    ) {
    ...
    candidate_indices[1] =  10;
    candidate_indices[2] =  20;
    candidate_indices[3] =  60;
    candidate_indices[4] =  90;
    ...
    {   
        ...
        std::cout << "--------------------candidate_indices.at(i)   -----------------------" <<  candidate_indices.at(i) << std::endl;

    //sc_debug
    std::cout  << "sc_matrix_local_map  =   "    <<std::endl;
    Eigen::MatrixXf   sc_matrix(candidate_scan_context.rows(), candidate_scan_context.cols());
    for(int did = 0; did < candidate_scan_context.rows();  ++ did){
        for(int sid = 0; sid < candidate_scan_context.cols(); ++sid){
            sc_matrix(did,sid)  =   candidate_scan_context(did,sid);
        }
    }
    if(candidate_indices.at(i) ==  60){
        CreateFile(sc_cur_scan,sc_cur_scan_path);        //  创建文件夹
        WriteText(sc_cur_scan, sc_matrix);
    }
    //sc_debug
        ...
        std::cout << "shift  =  "   <<  candidate_shift  << std::endl;
        std::cout << "dist  =  "    <<  candidate_dist  << std::endl;
...
bool ScanContextManager::LoadRingKeys(const std::string &input_path) {
    ...
    for (int i = 0; i < ring_keys.data_size(); ++i) {
        ...
        std::cout << "3  -ring_key  size  =  "  <<  state_.ring_key_.size()   << std::endl;
    }
